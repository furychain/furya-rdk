...
import (
	...
	"github.com/gridironxyz/rollapp/x/sequencers/types"
)
...
// get a single sequencer registered on furyint by consensus address
func (k Keeper) GetFuryintSequencerByAddr(ctx sdk.Context, consAddr sdk.ConsAddress) (power uint64, found bool) {
	store := ctx.KVStore(k.storeKey)
	powerByte := store.Get(types.GetFuryintSeqKey(consAddr))
	if powerByte == nil {
		return 0, false
	}
...
// get a single sequencer registered on furyint by consensus address
func (k Keeper) SetFuryintSequencerByAddr(ctx sdk.Context, validator stakingtypes.Validator) error {
	consAddr, err := validator.GetConsAddr()
	if err != nil {
		return err
	}
	...
	store := ctx.KVStore(k.storeKey)
	store.Set(types.GetFuryintSeqKey(consAddr), b)
	return nil
}
