...
SEQUENCER_AMOUNT=${SEQUENCER_AMOUNT:-10000000000ufury}
...
if [ "$SETTLEMENT_LAYER" = "furya" ] && [ ! "$(command -v $SETTLEMENT_EXECUTABLE)" ] ; then
  echo "$SETTLEMENT_EXECUTABLE does not exist"
  exit 1
fi

# Verify that a genesis file doesn't exists for the furya chain
...
$EXECUTABLE furyint unsafe-reset-all  --home "$ROLLAPP_CHAIN_DIR"
...
if [ "$SETTLEMENT_LAYER" = "furya" ]; then
    #add account for sequencer on the hub
    $SETTLEMENT_EXECUTABLE keys add "$KEY_NAME_FURY" --keyring-backend test --keyring-dir $KEYRING_PATH
    SEQ_ACCOUNT_ON_HUB="$($SETTLEMENT_EXECUTABLE keys show -a $KEY_NAME_FURY --keyring-dir $KEYRING_PATH --keyring-backend test)"
